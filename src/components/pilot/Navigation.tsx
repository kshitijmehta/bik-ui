...
import { getSubCategory, AppState, SubCategory } from 'reducers';
...
const Navigation: React.FunctionComponent = () => {
  ...
  const getCategoryAndSubCategory = () => {
    return Object.keys(SubCategories).map((key: string, index: number) => {
      return (
        <li key={index}>
          <a href="bindi-category.html">{key}<span className="uk-margin-xsmall-left" uk-icon="icon: chevron-down; ratio: .75;"></span></a>
          <div
              ...
              <ul className="uk-nav uk-nav-default uk-column-1-3">
                {
                  filterSubcategories(subCategories, Number(SubCategories[key])).map((subcategory: ProductSubCategory, index: number) => {
                    return <li key={index}><a href="bindi-category.html">{subcategory.value}</a></li>
                  })
                }
              </ul>
            </div>
